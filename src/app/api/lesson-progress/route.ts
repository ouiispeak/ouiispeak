...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      if (!user) {
        return NextResponse.json({ ok: true, skipped: 'not-authenticated' });
      }
      ...
      if (!mod) {
        return NextResponse.json({ ok: true, skipped: 'module-not-found' });
      }
      ...
      if (!lesson) {
        return NextResponse.json({ ok: true, skipped: 'lesson-not-found' });
      }
